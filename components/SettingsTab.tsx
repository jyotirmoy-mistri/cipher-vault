  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      <div className="p-5 bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm">
          <h3 className="font-bold flex items-center mb-4"><Palette className="w-5 h-5 mr-2 text-blue-500"/> QR Design</h3>
          <div className="flex space-x-4">
              <div className="flex-1">
                  <label className="text-xs text-gray-500 block mb-1">QR Color</label>
                  <input type="color" value={qrConfig.fg} onChange={(e)=>saveQrSettings(e.target.value, qrConfig.bg)} className="w-full h-10 rounded cursor-pointer" />
              </div>
              <div className="flex-1">
                  <label className="text-xs text-gray-500 block mb-1">Background</label>
                  <input type="color" value={qrConfig.bg} onChange={(e)=>saveQrSettings(qrConfig.fg, e.target.value)} className="w-full h-10 rounded cursor-pointer" />
              </div>
          </div>
      </div>
      ...
      <div className="p-6 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 rounded-3xl">
          ...
          <div className="flex justify-between items-center bg-white dark:bg-gray-950 p-3 rounded-lg border border-red-200 dark:border-red-900/50 mb-4">
              ...
              <button onClick={() => navigator.clipboard.writeText('DELETE')} className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 p-2"><Copy className="w-4 h-4"/></button>
          </div>
          ...
          <button onClick={async () => {
              ...
              await masterCacheClean(); window.location.reload();
          }} className="w-full p-4 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold">FORMAT VAULT</button>
      </div>
    </div>
  );
}
